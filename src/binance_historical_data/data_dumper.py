...
class FsTreeCache:
    def __init__(self):
        self._data = set()  # Use a set for O(1) lookups
    ...
    def scan(self, root_folder):
        ...
        for dirpath, _, filenames in os.walk(root_folder):
            for filename in filenames:
                ...
                self._data.add(filepath)

    def add(self, filepath):
        self._data.add(filepath)

    def exists(self, filepath):
        return filepath in self._data  # O(1) lookup in a set
...
class BinanceDataDumper:
    ...
    def __init__(
        ...
        self.tree = FsTreeCache()
        if fs_tree_cache_path and os.path.exists(fs_tree_cache_path):
            self.tree.load(fs_tree_cache_path)
        else:
            self.tree.scan(path_dir_where_to_dump)
